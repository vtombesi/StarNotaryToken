...
it('lets user2 buy a star and decreases its balance in ether', async() => {
    ...
    const gasPrice = Math.ceil(latestBlock.baseFeePerGas);
...
it('lets user2 buy a star and decreases its balance in ether', async() => {
    ...
    const gasPrice = Math.ceil(block.baseFeePerGas);
    await instance.buyStar(starId, {from: user2, value: value, gasPrice: gasPrice});
    const balanceAfterUser2BuysStar = await web3.eth.getBalance(user2);
    assert.isAbove(Number(balanceOfUser2BeforeTransaction), Number(balanceAfterUser2BuysStar));
});
